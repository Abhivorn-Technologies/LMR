...
import { riskManagementContent as defaultContent } from "@/lib/content/risk-management";
...
const getIcon = (iconName: string, fallback: any = Icons.ShieldCheck) => {
  if (!iconName) return fallback;
  const Component = (Icons as any)[iconName];
  return Component || fallback;
};

export function RiskManagementBlock({ content: rawContent }: { content: any }) {
  // Deep merge rawContent with defaultContent to ensure no broken fields
  const content = {
    hero: { ...defaultContent.hero, ...(rawContent?.hero || {}) },
    services: { ...defaultContent.services, ...(rawContent?.services || {}) },
    process: { ...defaultContent.process, ...(rawContent?.process || {}) },
    industries: { ...defaultContent.industries, ...(rawContent?.industries || {}) },
    whyChoose: { ...defaultContent.whyChoose, ...(rawContent?.whyChoose || {}) },
    cta: { ...defaultContent.cta, ...(rawContent?.cta || {}) }
  };

  const heroCards = content.hero.cards || defaultContent.hero.cards;
  const serviceItems = content.services.items || defaultContent.services.items;
  const processSteps = content.process.steps || defaultContent.process.steps;
  const industryItems = content.industries.items || defaultContent.industries.items;
  const reasons = content.whyChoose.reasons || defaultContent.whyChoose.reasons;
  const stats = content.whyChoose.stats || defaultContent.whyChoose.stats;
    ...
    <main className="w-full bg-white selection:bg-[#19C8D6] selection:text-white">
      ...
      <section className="relative w-full min-h-[90vh] flex items-center justify-center overflow-hidden bg-slate-900 pt-20">
        <div className="absolute inset-0 z-0">
          <Image 
            src={content.hero.image || "/assets/image6.jpeg"} 
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 w-full grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            {content.hero.badge && (
              <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#19C8D6]/10 border border-[#19C8D6]/20 mb-6 backdrop-blur-md">
                ...
                <span className="text-sm font-semibold tracking-wider text-[#19C8D6] uppercase">{content.hero.badge}</span>
              </motion.div>
            )}
            ...
            <motion.h1 variants={fadeUp} className="text-5xl md:text-6xl lg:text-7xl font-extrabold text-white mb-6 leading-[1.1] tracking-tight">
              {content.hero.titleLine1} <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#19C8D6] to-[#0E6D73]">{content.hero.titleHighlight}</span>
            </motion.h1>
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4">
              <Link href="/contact" className="inline-flex items-center justify-center px-8 py-4 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl backdrop-blur-md border border-white/10 transition-all duration-300 hover:-translate-y-1">
                {content.hero.ctaText || "Contact Experts"}
              </Link>
            </motion.div>
          </motion.div>
          ...
          >
            {heroCards.map((item: string, idx: number) => (
              <motion.div 
                key={item}
      ...
      <section className="py-24 bg-slate-50 relative overflow-hidden">
        <div className="max-w-[1400px] mx-auto px-6 relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {serviceItems.map((srv: any, i: number) => {
              const IconComp = getIcon(srv.icon, Icons.Building2);
              return (
                <motion.div 
                  key={srv.title}
        ...
        <div className="max-w-[1400px] mx-auto px-6">
          ...
          <div className="relative flex flex-col md:flex-row justify-between items-center md:items-start gap-8 md:gap-4">
            ...
            {processSteps.map((step: any, i: number) => {
              const StepIcon = getIcon(step.icon, Icons.LineChart);
              return (
                <motion.div 
                  key={step.num || i}
        ...
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            ...
            <Link href="/industries" className="shrink-0 flex items-center gap-2 text-[#19C8D6] font-bold hover:text-white transition-colors">
              View All Industries <Icons.ArrowRight size={20} />
            </Link>
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {industryItems.map((ind: any, i: number) => (
              <motion.div 
                key={ind.title || i}
      ...
      <section className="py-24 bg-white">
        <div className="max-w-[1400px] mx-auto px-6 mb-24">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <Image src={content.whyChoose.image || "/assets/image6.jpeg"} alt="Corporate Meeting" fill className="object-cover" />
            </motion.div>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-6 mb-12">
                {reasons.map((r: string, i: number) => (
                  ...
                  >
                    <Icons.CheckCircle2 className="text-[#19C8D6] shrink-0 mt-0.5" size={24} />
        ...
        <div className="max-w-[1200px] mx-auto px-6">
          ...
          >
            <div className="absolute inset-0">
              <Image src={content.cta.bgImage || "/assets/image5.jpeg"} alt="CTA Background" fill className="object-cover opacity-20 mix-blend-overlay" />
